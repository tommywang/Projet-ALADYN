package fr.upmc.dtgui.junit.robot;

import org.junit.Test;

import fr.upmc.dtgui.javassist.robot.SteeringChangeJavassist;
import junit.framework.TestCase;
import javassist.ClassPool;
import javassist.CtClass;


public class RobotSteeringChangeTest extends TestCase {
		@Override
		public void setUp() throws Exception {
			pool = ClassPool.getDefault();
			littleRobot=pool.get("fr.upmc.dtgui.tests.LittleRobot");
			SteeringChangeJavassist scj=new SteeringChangeJavassist();
			scj.create(pool, littleRobot);
		}

		private CtClass littleRobot;
		private ClassPool pool;
		private boolean classFound=true;

		@Test
		public void testMakeClass() 
				throws Throwable{
			try{
				pool.get("fr.upmc.dtgui.tests.LittleRobot$SteeringChange");
			}
			catch(Exception e){
				classFound=false;
				throw new Exception(e.toString());
			}
			assertTrue(classFound);
		}
}
